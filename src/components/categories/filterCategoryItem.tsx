...
const FilterCategoryItem = () => {
	...
	const fetchOneItem = async (token: number) => {
		...
		try {
			const response = await fetch(`${urlV3}/getNFTsForContract?contractAddress=${contract_address}&withMetadata=true&startToken=${token}&limit=${token}`, options);
	...
	return (
		<div className="flex flex-col justify-center items-center">
			...
			></button>
			<div className='flex flex-col justify-center items-center'>
				{searchInput.length > 0 ? <OneCategoryItem item={searchResult} /> : <CategoryItem />}
			</div>
		</div>
	);
};
