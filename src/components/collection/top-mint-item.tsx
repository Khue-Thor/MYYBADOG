...
const TopMintItem = async (props: {
  index: number;
  data: TopMintCollectionRecord;
}) => {
  ...
  return (
    <>
      ...
      >
        ...
        <div>
          <Link href={linkPath} className="block" prefetch={false}>
            <span className="font-display text-jacarta-700 hover:text-accent font-semibold dark:text-white">
              {collection_name}
            </span>
          </Link>
          <span className="dark:text-jacarta-300 text-sm">
            {Math.round(mint_num * mint_volume.value)} {mint_volume.crypto_unit}
          </span>
        </div>
      </div>
      {/* <!-- end top mint item --> */}
