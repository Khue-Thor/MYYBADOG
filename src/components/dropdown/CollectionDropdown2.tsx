...
type Props = {
  data: { id: string; text: string; image: string }[];
  collection?: boolean;
};

const CollectionDropdown2: React.FC<Props> = ({ data, collection }) => {
  const [dropdown, setDropdown] = useState(false);
  const [activeItem, setActiveItem] = useState<string | null>(null);

  const handleDropdown = () => {
    window.addEventListener("click", (e) => {
      if ((e.target as Element).closest(".dropdown-toggle")) {
        if (dropdown) {
          setDropdown(false);
        } else {
          setDropdown(true);
...
export default CollectionDropdown2;
