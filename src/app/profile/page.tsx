...
import UserId from "@/app/profile/UserId";
import defaultProfileImage from "@/public/images/avatars/avatar_1.jpg";
import defaultCoverImage from "@/public/images/case-studies/single_case_study_featured.jpg";

const Profile = () => {
  const [preview, setPreview] = useState<string | null>();
  const [coverPreview, setCoverPreview] = useState<string | null>();

  const handleProfilePhoto: ChangeEventHandler<HTMLInputElement> = (e) => {
  ...
  const handleCoverPhoto: ChangeEventHandler<HTMLInputElement> = (e) => {
    if (!e.target.files) return;
    const file = e.target.files[0];
    if (!file) return;
    const reader = new FileReader();
      ...
      <div className="pt-[5.5rem] lg:pt-24">
        ...
        <div className="relative h-[18.75rem]">
          <Image
            fill
            sizes="100vw"
            src={coverPreview || defaultCoverImage}
            alt="banner"
            className="h-[18.75rem] w-full object-cover"
          />
          <div className="absolute w-full bottom-4">
            <div className="container flex justify-end">
              <button className="font-display group hover:bg-accent flex items-center rounded-lg bg-white py-2 px-4 text-sm">
                <input
                  type="file"
                  accept="image/*"
                  ...
                  <figure className="relative inline-block">
                    <div className="relative w-[140px] h-[140px]">
                      <Image
                        ...
                        fill
                        className="dark:border-jacarta-600 rounded-xl border-[5px] border-white object-cover"
                      />
                    </div>
